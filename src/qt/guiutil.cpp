...
#include "bitcoinplanetaddressvalidator.h"
...
QFont bitcoinplanetAddressFont()
{
...
{
    widget->setMaxLength(BitcoinPlanetAddressValidator::MaxAddressLength);
    widget->setValidator(new BitcoinPlanetAddressValidator(parent));
    widget->setFont(bitcoinplanetAddressFont());
}
...
bool parseBitcoinPlanetURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("bitcoinplanet"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinPlanetUnits::parse(BitcoinPlanetUnits::BTPL, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitcoinPlanetURI(QString uri, SendCoinsRecipient *out)
{
    // Convert bitcoinplanet:// to bitcoinplanet:
    //
    //    Cannot handle this later, because bitcoinplanet:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitcoinplanet://"))
    {
        uri.replace(0, 12, "bitcoinplanet:");
    }
    QUrl uriInstance(uri);
    return parseBitcoinPlanetURI(uriInstance, out);
}
...
void openConfigfile()
{
    ...
    /* Open bitcoinplanet.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinPlanet.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinPlanet.lnk
...
{
    return GetAutostartDir() / "bitcoinplanet.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a bitcoinplanet.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BitcoinPlanet\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BitcoinPlanet-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bitcoinplanet-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BitcoinPlanet-Qt"));
