...
{
    QString ret = QString("bitcoinplanet:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BTPL unit input in lnReqAmount, we generate the URI with BTPL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinPlanetUnits::format(BitcoinPlanetUnits::BTPL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
