...
#if defined(NDEBUG)
# error "BitcoinPlanet cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BitcoinPlanet Signed Message:\n";
...
{
    int64_t nSubsidy = 20 * COIN; // DigitalMoneyBits base reward

    // Subsidy is cut in half every ~4 years
    nSubsidy >>= (nBestHeight / 4905600);
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Modified for BitcoinPlanet: softfork block set to min diff to re-engage chain movement
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    //LogPrintf("BitcoinPlanet retargetted using: DGW-v3 difficulty algo \n");
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal bitcoinplanet miner
...
#ifdef ENABLE_WALLET
// bitcoinplanet: attempt to generate suitable proof-of-stake
...
{
    RenameThread("bitcoinplanet-loadblk");
