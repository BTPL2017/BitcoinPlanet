...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("bitcoinplanet-miner");
...
void static BitcoinPlanetMiner(CWallet *pwallet)
{
    LogPrintf("BitcoinPlanetMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinplanet-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running BitcoinPlanetMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("BitcoinPlanetMiner terminated\n");
        throw;
    }
}

void GenerateBitcoinPlanets(bool fGenerate, CWallet* pwallet, int nThreads)
{
	...
	minerThreads->create_thread(boost::bind(&BitcoinPlanetMiner, pwallet));
}
#endif
